#!/usr/bin/env python3
"""Analyze experiment 3 results and output structured data for TeX import.
...
def analyze_showcase():
    ...
    tool_notes = {
        "CensorLang": "regex on raw payload (no TLS parsing)",
    ...
    with open(table4_json, "w") as f:
        json.dump({"table": "table4", "experiment": "03_tls_sni", "rows": rows}, f, indent=2)
    ...
    if os.path.exists(labels_path) and os.path.exists(scapy_decisions):
        ...
        tpr, tnr, tp, fn, tn, fp = compute_tpr_tnr(labels, decisions)
...
def analyze_live_eval(results_csv):
    ...
    with open(table3_json, "w") as f:
        json.dump({"table": "table3", "experiment": "03_tls_sni", "rows": rows}, f, indent=2)
...
def main():
    ...
    if not has_showcase and not args.live_eval:
        ...
        print("  bash experiments/03_tls_sni/scripts/run_showcase.sh")
        print("  bash experiments/03_tls_sni/scripts/run_evaluation.sh")
