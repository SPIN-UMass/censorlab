...
#
# Runs all tools (PyCL, CensorLang, Zeek, Scapy) against the DNS test PCAP
...
#   USE_DOCKER=1 bash experiments/02_dns_injection/scripts/run_showcase.sh [ITERATIONS]
...
echo "=== Experiment 2: DNS Response Injection — Showcase (Table 4) ==="
echo "PCAP: $PCAP"
...
if command -v zeek &> /dev/null; then
    ...
    for i in $(seq 1 "$ITERATIONS"); do
        ...
        start_time=$(date +%s%N)
        zeek -C -r "$PCAP" "$EXPERIMENT_DIR/comparison/zeek_dns_detection.zeek" 2>&1 || true
        end_time=$(date +%s%N)
        elapsed_us=$(( (end_time - start_time) / 1000 ))
        echo "Zeek,$i,$elapsed_us" >> "$RAW_TIMINGS"
        echo "  Run $i: ${elapsed_us}us"
        if [ "$i" -eq 1 ] && [ -f dns_injection_matches.log ]; then
            cp dns_injection_matches.log "$RESULTS_DIR/zeek_matches.log"
        fi
        rm -f *.log
        popd > /dev/null
    done
...
for i in $(seq 1 "$ITERATIONS"); do
    python3 "$EXPERIMENT_DIR/comparison/scapy_dns_injection.py" "$PCAP" \
...
zeek_loc=$(count_loc "$EXPERIMENT_DIR/comparison/zeek_dns_detection.zeek")
echo "Zeek,zeek_dns_detection.zeek,$zeek_loc" >> "$LOC_FILE"
echo "  Zeek (zeek_dns_detection.zeek): $zeek_loc"

scapy_loc=$(count_loc "$EXPERIMENT_DIR/comparison/scapy_dns_injection.py")
echo "Scapy,scapy_dns_injection.py,$scapy_loc" >> "$LOC_FILE"
echo "  Scapy (scapy_dns_injection.py): $scapy_loc"
