#!/usr/bin/env python3
"""Analyze experiment 2 results and output structured data for TeX import.
...
def analyze_showcase():
    ...
    tool_notes = {
        "CensorLang": "drop-only (no injection)",
        "Zeek": "detection-only (passive)",
    }
    ...
    with open(table4_json, "w") as f:
        json.dump({"table": "table4", "experiment": "02_dns_injection", "rows": rows}, f, indent=2)
    ...
    if os.path.exists(labels_path) and os.path.exists(scapy_decisions):
        ...
        decisions = load_decisions(scapy_decisions)
        tpr, tnr, tp, fn, tn, fp = compute_tpr_tnr(labels, decisions, positive_actions=("inject",))
...
def analyze_live_eval(results_csv):
    ...
    tp = sum(1 for r in results if r["class"] == "forbidden" and r["outcome"] == "injected")
    fn = sum(1 for r in results if r["class"] == "forbidden" and r["outcome"] != "injected")
    tn = sum(1 for r in results if r["class"] == "allowed" and r["outcome"] == "normal")
    fp = sum(1 for r in results if r["class"] == "allowed" and r["outcome"] == "injected")
    ...
    rows = [
        ...
        },
        {
            ...
            "errors": None,
            "total": None,
            "note": "drop-only (cannot inject DNS responses)",
        },
    ]
    ...
    with open(table3_json, "w") as f:
        json.dump({"table": "table3", "experiment": "02_dns_injection", "rows": rows}, f, indent=2)
...
def main():
    ...
    if not has_showcase and not args.live_eval:
        ...
        print("  bash experiments/02_dns_injection/scripts/run_showcase.sh")
