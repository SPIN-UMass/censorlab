#!/usr/bin/env python3
"""Analyze experiment 4 results and output structured data for TeX import.
...
def analyze_showcase():
    ...
    with open(table4_json, "w") as f:
        json.dump({"table": "table4", "experiment": "04_shadowsocks", "rows": rows}, f, indent=2)
    ...
    if os.path.exists(labels_path) and os.path.exists(scapy_decisions):
        ...
        tpr, tnr, tp, fn, tn, fp = compute_tpr_tnr(labels, decisions, positive_actions=("drop", "reset"))
...
def analyze_live_eval(results_csv):
    ...
    rows = [
        {
            ...
            "errors": errors,
            "total": len(results),
            "note": "Wu et al. 2023 heuristics (all rules)",
        },
        {
            "tool": "CensorLang",
            "tpr": None,
    ...
    with open(table3_json, "w") as f:
        json.dump({"table": "table3", "experiment": "04_shadowsocks", "rows": rows}, f, indent=2)
...
def main():
    ...
    if not has_showcase and not args.live_eval:
        ...
        print("  bash experiments/04_shadowsocks/scripts/run_showcase.sh")
