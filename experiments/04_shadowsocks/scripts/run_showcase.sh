...
#
# Runs all tools (PyCL, CensorLang, Zeek, Scapy) against the Shadowsocks
# detection test PCAP multiple times and collects timing results.
#
...
echo "=== Experiment 4: Shadowsocks/Encrypted Protocol Detection — Showcase (Table 4) ==="
echo "PCAP: $PCAP"
...
if command -v zeek &> /dev/null; then
    ...
    for i in $(seq 1 "$ITERATIONS"); do
        ...
        start_time=$(date +%s%N)
        zeek -C -r "$PCAP" "$EXPERIMENT_DIR/comparison/zeek_shadowsocks.zeek" 2>&1 || true
        end_time=$(date +%s%N)
        elapsed_us=$(( (end_time - start_time) / 1000 ))
        echo "Zeek,$i,$elapsed_us" >> "$RAW_TIMINGS"
        echo "  Run $i: ${elapsed_us}us"
        # Save first-run logs
        if [ "$i" -eq 1 ] && [ -f shadowsocks_matches.log ]; then
            cp shadowsocks_matches.log "$RESULTS_DIR/zeek_matches.log"
        fi
        rm -f *.log
        popd > /dev/null
    done
...
for i in $(seq 1 "$ITERATIONS"); do
    python3 "$EXPERIMENT_DIR/comparison/scapy_shadowsocks.py" "$PCAP" \
        --output "$RESULTS_DIR/scapy_decisions.csv" 2>&1 | tee /tmp/scapy_run.txt
    # scapy_shadowsocks.py writes timing (seconds) to scapy_timing.txt
...
zeek_loc=$(count_loc "$EXPERIMENT_DIR/comparison/zeek_shadowsocks.zeek")
echo "Zeek,zeek_shadowsocks.zeek,$zeek_loc" >> "$LOC_FILE"
echo "  Zeek (zeek_shadowsocks.zeek): $zeek_loc"

scapy_loc=$(count_loc "$EXPERIMENT_DIR/comparison/scapy_shadowsocks.py")
echo "Scapy,scapy_shadowsocks.py,$scapy_loc" >> "$LOC_FILE"
echo "  Scapy (scapy_shadowsocks.py): $scapy_loc"
